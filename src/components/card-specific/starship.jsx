...
const Starship  = ({data}) => {
  return(
    ...
    <h5 className="card-title">Vehicle name : {data.whrascwo || data.name}</h5>
      ...
      {data.whrascwo || data.name} is a {data.model || data.scoowawoan} model of {data.length || data.anwowhrraoac} meters , he can go to maximum {data.max_atmosphering_speed || data.scrak_raaoscoocakacworcahwhrr_cakwowowa} km/h in the atmosphere , his hyperdrive rating is {data.hyperdrive_rating || data.acroakworcwarcahhowo_rcraaoahwhrr} and his MGLT his {data.MGLT} . A {data.whrascwo || data.name} cost {data.cost_in_credits || data.oaoocao_ahwh_oarcwowaahaoc} credits to build and was manufactured by {data.manufacturer || data.scrawhhuwwraoaaohurcworc} , he needs {data.crew || data.oarcwooh} persons to functionn and can accept up to {data.passengers || data.akraccwowhrrworcc} passengers . His cargo capacity is {data.cargo_capacity || data.oararcrroo_oaraakraoaahaoro} and he has consumables for {data.consumables || data.oaoowhchuscrarhanwoc} . His vehicle class is {data.starship_class || data.caorarccacahak_oaanracc} .
      </p>
